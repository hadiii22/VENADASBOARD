...
const Settings: React.FC<SettingsProps> = ({ profile, setProfile, transactions, projects }) => {
    ...
    const handleAddOrUpdateIncomeCategory = () => {
        ...
        if (editingIncomeCategory) { // Update
            ...
            setProfile(prev => ({
                ...prev,
                incomeCategories: categories.map(c => c === editingIncomeCategory ? newCategory : c).sort()
            }));
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="fullName" value={profile.fullName} onChange={handleInputChange} className="mt-1 input-field" />
                            </div>
                             <div>
                                <label className="block text-sm font-medium text-slate-700">Email</label>
                                <input type="email" name="email" value={profile.email} onChange={handleInputChange} className="mt-1 input-field" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700">Telepon</label>
                                <input type="text" name="phone" value={profile.phone} onChange={handleInputChange} className="mt-1 input-field" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700">Nama Perusahaan</label>
                                <input type="text" name="companyName" value={profile.companyName} onChange={handleInputChange} className="mt-1 input-field" />
                            </div>
                            <div className="md:col-span-2">
                                <label className="block text-sm font-medium text-slate-700">Website</label>
                                <input type="url" name="website" value={profile.website} onChange={handleInputChange} className="mt-1 input-field" />
                            </div>
                            <div className="md:col-span-2">
                                <label className="block text-sm font-medium text-slate-700">Alamat</label>
                                <input type="text" name="address" value={profile.address} onChange={handleInputChange} className="mt-1 input-field" />
                            </div>
                            <div className="md:col-span-2">
                                <label className="block text-sm font-medium text-slate-700">Rekening Perusahaan</label>
                                <input type="text" name="bankAccount" placeholder="e.g., BCA 1234567890 a/n Perusahaan Anda" value={profile.bankAccount} onChange={handleInputChange} className="mt-1 input-field" />
                            </div>
                             <div className="md:col-span-2">
                                <label className="block text-sm font-medium text-slate-700">Bio</label>
                                <textarea name="bio" rows={3} value={profile.bio} onChange={handleInputChange} className="mt-1 input-field"></textarea>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="space-y-2 max-h-48 overflow-y-auto p-1">
                                    {(profile.projectTypes || []).map(cat => renderCategoryItem(cat, handleEditProjectType, handleDeleteProjectType))}
                                </div>
                            </div>
                            {/* Event Types */}
                            <div className="space-y-4">
                                ...
                                <div className="space-y-2 max-h-48 overflow-y-auto p-1">
                                    {(profile.eventTypes || []).map(cat => renderCategoryItem(cat, handleEditEventType, handleDeleteEventType))}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="space-y-2 max-h-48 overflow-y-auto p-1">
                                    {(profile.incomeCategories || []).map(cat => renderCategoryItem(cat, handleEditIncomeCategory, handleDeleteIncomeCategory))}
                                </div>
                            </div>
                            {/* Expense Categories */}
                            <div className="space-y-4">
                                ...
                                <div className="space-y-2 max-h-48 overflow-y-auto p-1">
                                    {(profile.expenseCategories || []).map(cat => renderCategoryItem(cat, handleEditExpenseCategory, handleDeleteExpenseCategory))}
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="max-w-2xl mx-auto">
                        ...
                        <div className="space-y-5 divide-y divide-slate-200">
                           <div className="flex items-center justify-between pt-5 first:pt-0">
                                ...
                                <ToggleSwitch enabled={profile.notificationSettings.newProject} onChange={() => setProfile(p => ({ ...p, notificationSettings: {...p.notificationSettings, newProject: !p.notificationSettings.newProject} }))} />
                           </div>
                           <div className="flex items-center justify-between pt-5 first:pt-0">
                                <div>
                                ...
                                <ToggleSwitch enabled={profile.notificationSettings.paymentConfirmation} onChange={() => setProfile(p => ({ ...p, notificationSettings: {...p.notificationSettings, paymentConfirmation: !p.notificationSettings.paymentConfirmation} }))} />
                           </div>
                           <div className="flex items-center justify-between pt-5 first:pt-0">
                                <div>
                                ...
                                <ToggleSwitch enabled={profile.notificationSettings.deadlineReminder} onChange={() => setProfile(p => ({ ...p, notificationSettings: {...p.notificationSettings, deadlineReminder: !p.notificationSettings.deadlineReminder} }))} />
                           </div>
                        </div>
                    </div>
                );
                        ...
                        <div>
                            ...
                            <div className="space-y-5 divide-y divide-slate-200">
                                <div className="flex items-center justify-between pt-5 first:pt-0">
                                    ...
                                    <ToggleSwitch enabled={profile.securitySettings.twoFactorEnabled} onChange={() => setProfile(p => ({ ...p, securitySettings: {...p.securitySettings, twoFactorEnabled: !p.securitySettings.twoFactorEnabled} }))} />
                                </div>
                            </div>
                        </div>
                        <div>
